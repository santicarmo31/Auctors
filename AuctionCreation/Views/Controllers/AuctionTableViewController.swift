...
class AuctionTableViewController: UITableViewController {
    ...
    */
    ...
    @IBAction func saveAuction(_ sender: Any) {
        guard let cell = tableView.visibleCells.first as? AuctionTableViewCell else {
            return
        }
        let milliseconds = cell.defaultPickerView.date.millisecondsSince1970
        viewModel?.auctionResponse.auction.update(actualEndDate: Int(milliseconds), description: cell.descriptionTextfield.text!, quantity: Int(cell.quantityTextfield.text!) ?? 0, title: cell.titleTextfield.text!)
        presenter.post(response: viewModel!.auctionResponse)
    }
